use crate::bus::{AccessSize, Bus, Clint, ClintState, MemoryFault, MmioDevice, Ram, Uart};
...
fn register_platform_devices(
    ...
    bus.add_device(
        crate::config::CLINT_BASE,
        crate::config::CLINT_SIZE,
        Box::new(Clint::new(clint_state)),
    );
    bus.add_device(
        crate::config::PLIC_BASE,
        crate::config::PLIC_SIZE,
        Box::new(MmioDevice),
    );
    bus.add_device(
        crate::config::UART_BASE,
        crate::config::UART_SIZE,
        Box::new(Uart::new(uart_output)),
    );
}
...
impl Processor {
    pub fn new() -> Self {
        ...
        let mut bus = Bus::new();
        let clint_state = Arc::new(Mutex::new(ClintState {
            mtime: 0,
            mtimecmp: u64::MAX,
        }));
    ...
    fn fetch(&self) -> Result<u32, StepError> {
        self.bus
            .read(self.pc, AccessSize::Word)
            .map_err(|e| StepError::MemoryFault(e))
    }
    ...
    fn execute(&mut self, instruction: Instruction) -> Result<(), StepError> {
        ...
        match instruction {
            ...
            Instruction::Sub { rd, rs1, rs2 } => {
                let result = self
                    .read_register(rs1)
                    .wrapping_sub(self.read_register(rs2));
                self.write_register(rd, result);
            }
            ...
            Instruction::Slt { rd, rs1, rs2 } => {
                let result = if (self.read_register(rs1) as i32) < (self.read_register(rs2) as i32)
                {
                    1
                } else {
                    0
            ...
            Instruction::Sltu { rd, rs1, rs2 } => {
                let result = if self.read_register(rs1) < self.read_register(rs2) {
                    1
                } else {
                    0
                };
            ...
            Instruction::Slti { rd, rs1, imm } => {
                let result = if (self.read_register(rs1) as i32) < imm {
                    1
                } else {
                    0
                };
            ...
            Instruction::Sltiu { rd, rs1, imm } => {
                let result = if self.read_register(rs1) < imm as u32 {
                    1
                } else {
                    0
                };
            ...
            Instruction::Sb { rs1, rs2, imm } => {
                ...
                let address = self.read_register(rs1).wrapping_add(imm as u32);
                self.bus
                    .write(address, self.read_register(rs2), AccessSize::Byte)?;
            }
            Instruction::Sh { rs1, rs2, imm } => {
                // M[rs1+imm][0:15] = rs2[0:15]
                let address = self.read_register(rs1).wrapping_add(imm as u32);
                self.bus
                    .write(address, self.read_register(rs2), AccessSize::Half)?;
            }
            Instruction::Sw { rs1, rs2, imm } => {
                // M[rs1+imm][0:31] = rs2[0:31]
                let address = self.read_register(rs1).wrapping_add(imm as u32);
                self.bus
                    .write(address, self.read_register(rs2), AccessSize::Word)?;
            }
...
mod tests {
    ...
    fn test_load_out_of_bounds_returns_fault() {
        ...
        let result = p.execute(Instruction::Lw {
            rd: 2,
            rs1: 1,
            imm: 0,
        });
        assert!(matches!(
            result,
            Err(StepError::MemoryFault(MemoryFault::OutOfBounds {
                address: 0x20000000
            }))
        ));
    }
    ...
    fn test_store_out_of_bounds_returns_fault() {
        let mut p = processor_with_data(vec![0x00]);
        p.write_register(1, 0x20000000); // unmapped address
        let result = p.execute(Instruction::Sb {
            rs1: 1,
            rs2: 2,
            imm: 0,
        });
    ...
    fn test_clint_mtimecmp_readable_via_bus() {
        ...
        let lo = p
            .bus
            .read(crate::config::CLINT_BASE + 0x4000, AccessSize::Word)
            .unwrap();
        let hi = p
            .bus
            .read(crate::config::CLINT_BASE + 0x4004, AccessSize::Word)
            .unwrap();
    ...
    fn test_uart_lsr_readable_via_bus() {
        ...
        let lsr = p
            .bus
            .read(crate::config::UART_BASE + 5, AccessSize::Byte)
            .unwrap();
